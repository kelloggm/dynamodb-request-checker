...
/** This class contains all the hard-coded business logic related to DynamoDB. */
public final class DDBUtils {

  public static final @CompilerMessageKey String MIXED_NAMES_VALUES =
      "ddb.names.and.values.confused";
  ...
  private static final String DDBV2QRB =
  ...
  public DDBUtils(ProcessingEnvironment processingEnv) {
    ...
    namesSetters.add(
        TreeUtils.getMethod(DDBV1RQ, "withExpressionAttributeNames", 1, processingEnv));
    valuesSetters = new ArrayList<>();
    valuesSetters.add(TreeUtils.getMethod(DDBV2QRB, "expressionAttributeValues", 1, processingEnv));
    valuesSetters.add(
        TreeUtils.getMethod(DDBV1RQ, "withExpressionAttributeValues", 1, processingEnv));
    expressionSetters = new ArrayList<>();
    expressionSetters.add(
        TreeUtils.getMethod(DDBV2QRB, "keyConditionExpression", 1, processingEnv));
    expressionSetters.add(TreeUtils.getMethod(DDBV2QRB, "filterExpression", 1, processingEnv));
    expressionSetters.add(
        TreeUtils.getMethod(DDBV1RQ, "withKeyConditionExpression", 1, processingEnv));
  ...
  /**
   * Returns true if this method is white-listed as returning its receiver. This is a bit of an ugly
   * hack, but it definitely works.
   */
  ...
  public boolean isExpressionMethodCall(
      MethodInvocationNode node, ProcessingEnvironment processingEnv) {
