...
public class MapUtils {
    public MapUtils(ProcessingEnvironment processingEnv) {
        ...
        for (int ofParamCount = 2; ofParamCount <= 10; ofParamCount += 2) {
            immutableMapOfs.add(TreeUtils.getMethod("com.google.common.collect.ImmutableMap",
                    "of", ofParamCount, processingEnv));
        }
        for (int ofParamCount = 2; ofParamCount <= 10; ofParamCount += 2) {
            immutableMapOfs.add(TreeUtils.getMethod("software.amazon.awssdk.utils.ImmutableMap",
                    "of", ofParamCount, processingEnv));
        }
        immutableMapOfs.add(TreeUtils.getMethod("java.util.Collections",
                "singletonMap", 2, processingEnv));
        ...
        immutableMapBuilderPuts.add(TreeUtils.getMethod("com.google.common.collect.ImmutableMap.Builder",
                "put", 2, processingEnv));
        immutableMapBuilderPuts.add(TreeUtils.getMethod("software.amazon.awssdk.utils.ImmutableMap.Builder",
                "put", 2, processingEnv));
    }
    ...
    /**
     *  The representations of methods like com.google.common.collect.ImmutableMap.Builder#put,
     *  which both act like Map#put *and* return their receiver.
     */
    private final List<ExecutableElement> immutableMapBuilderPuts;

    /**
     * This list contains representations of all methods that should be treated like
     * the com.google.common.collect.ImmutableMap#of methods.
     */
    private final List<ExecutableElement> immutableMapOfs;

    /**
     * Returns true if the given MethodInvocationTree is invoking a method that
     * should be used for inference in the style of Map#put, in the given processing environment.
     * The processing environment should come from an AnnotatedTypeFactory.
     */
    ...
    /**
     * Returns true iff this method is defined in a builder for a map
     * similar to com.google.common.collect.ImmutableMap and
    ...
    public boolean isFromImmutableMapBuilder(MethodInvocationTree tree) {
        ...
        if (enclosingClass != null) {
            for (String builderName : IMMUTABLE_MAP_BUILDERS) {
                if (builderName.equals(qualifiedName) &&
                        builderName.equals(returnTypeName)) {
                    return true;
                }
            }
        }
    ...
    /**
     * Checks whether the given tree is an invocation of a method that should be treated like ImmutableMap#of.
     */
    ...
    /**
     * Checks whether the given tree is an invocation of a method that should be treated like ImmutableMap.Builder#put.
     */
    public boolean isImmutableMapBuilderPut(MethodInvocationTree tree, ProcessingEnvironment processingEnv) {
