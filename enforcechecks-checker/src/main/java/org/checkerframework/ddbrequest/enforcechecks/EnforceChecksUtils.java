...
public class EnforceChecksUtils {
    ...
    /**
     * Checks that the types given by the type factories provided for the definitions
     * and requirements are correct, and issues an error through the checker's reporting
     * interface otherwise. This version uses the same tree to report that it checks.
     */
    public static void checkRequirementsAgainstDefinitions(
        ...
        checkRequirementsAgainstDefinitions(treeToCheck,
                treeToCheck, definitionFactory, requirementsFactory, checker);
    }

    /**
     * Checks that the types given by the type factories provided for the definitions
     * and requirements are correct, and issues an error through the checker's reporting
     * interface otherwise. This version uses different trees to check and on which to report.
     */
    public static void checkRequirementsAgainstDefinitions(
            Tree treeToCheck,
            Tree reportLocation,
        ...
        List<String> requiredNames = AnnotationUtils.areSame(requirementsAnno, requirementsFactory.top) ?
                new ArrayList<>() : AnnotationUtils.getElementValueArray(requirementsAnno,
                "names", String.class, true);

        List<String> requiredValues = AnnotationUtils.areSame(requirementsAnno, requirementsFactory.top) ?
                new ArrayList<>() : AnnotationUtils.getElementValueArray(requirementsAnno,
                "values", String.class, true);
        ...
        List<String> definitionNames = AnnotationUtils.areSame(definitionsAnno, definitionFactory.top) ?
                new ArrayList<>() : AnnotationUtils.getElementValueArray(definitionsAnno,
                "names", String.class, true);

        List<String> definitionValues = AnnotationUtils.areSame(definitionsAnno, definitionFactory.top) ?
                new ArrayList<>() : AnnotationUtils.getElementValueArray(definitionsAnno,
                "values", String.class, true);

        // Then turn these two lists into hashsets with the actual names, stripping the # and : in the process.
        ...
        for (String name : definitionNames) {
            if (name.startsWith("#")) {
                ...
                checker.report(Result.failure(DDBUtils.MIXED_NAMES_VALUES, name, "expressionAttributeNames"), reportLocation);
            }
        }
        ...
        for (String value : definitionValues) {
            if (value.startsWith(":")) {
                ...
                checker.report(Result.failure(DDBUtils.MIXED_NAMES_VALUES, value, "expressionAttributeValues"), reportLocation);
            }
        }
        ...
        for (String requiredName : requiredNames) {
            if (!definedNames.contains(requiredName)) {
                ...
                checker.report(Result.failure(DDBUtils.UNDEFINED, "name", requiredName, definedNamesString), reportLocation);
            }
        }
        ...
        for (String requiredValue : requiredValues) {
            if (!definedValues.contains(requiredValue)) {
                ...
                checker.report(Result.failure(DDBUtils.UNDEFINED, "value", requiredValue, definedValuesString), reportLocation);
            }
        }
    }
}
