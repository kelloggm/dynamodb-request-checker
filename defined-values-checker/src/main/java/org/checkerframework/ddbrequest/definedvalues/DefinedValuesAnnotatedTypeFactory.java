...
import org.checkerframework.ddbrequest.definedvalues.qual.DefinedValuesBottom;
...
public class DefinedValuesAnnotatedTypeFactory extends AccumulationAnnotatedTypeFactory {

  /** The business logic related to which DynamoDB operations to support. */
  private final DDBUtils ddbUtils;

  public DefinedValuesAnnotatedTypeFactory(BaseTypeChecker checker) {
    super(checker, DefinedValues.class, DefinedValuesBottom.class);
  ...
  public AnnotationMirror createDDBValuesDefinitions(Collection<String> values) {
    AnnotationBuilder builder = new AnnotationBuilder(processingEnv, DefinedValues.class);
  ...
  private class DDBValuesDefinitionsTreeAnnotator extends TreeAnnotator {
    DDBValuesDefinitionsTreeAnnotator(DefinedValuesAnnotatedTypeFactory atf) {
      super(atf);
    }
